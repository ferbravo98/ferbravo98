<h1 align="center">Hola 👋, soy Fernando Bravo</h1>
<h3 align="center">Empleado de Farmacia | Apasionada por la Tecnología</h3>

---

🔬 Actualmente soy **Empleado de Farmacia**, tambien me encuentro estudiando la carrera de la Licenciatura en Informatica, cursando el 3er año de la carrera

🧪 Estoy explorando el mundo del desarrollo de software para crear soluciones prácticas en el ámbito de la salud, como apps para el control de turnos o gestión de stock.

💻 Actualmente estoy aprendiendo **Python** para automatizar tareas y crear herramientas útiles en el entorno profesional.

📦 Algunos proyectos recientes:
- 💊 App de cierre de caja para farmacias (en desarrollo)
- 🩺 Sistema de gestión de pacientes con interfaz gráfica (Tkinter + MySQL)

🌱 Habilidades:
- Gestión farmacéutica y administrativa
- Buen manejo de bases de datos (MySQL)
- Conocimientos básicos de programación en Python
- Comunicación y trabajo en equipo

📫 Cómo contactarme:
- 📧 Email: bravofernando98@gmail.com
- 💼 LinkedIn: www.linkedin.com/in/fernando-nicolas-bravo-b42b54286

---

🧠 Siempre en busca de aprender algo nuevo y aportar valor con herramientas prácticas.

⭐ ¡Gracias por visitar mi perfil!
